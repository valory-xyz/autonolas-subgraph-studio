...
- [`subgraphs/pearl/SUBGRAPH_PLAN.md`](subgraphs/pearl/SUBGRAPH_PLAN.md) — Pearl Mini analytics via in-place generalization of `predict-polymarket` (Polygon). Cohort membership resolved on-chain via `ApplicationClassifier` (from `valory-xyz/autonolas-registries`); `service-registry` reused for service enumeration; no dedicated Pearl subgraph. Documents on-chain-only constraint, NegRisk coverage, ROI handling, out-of-subgraph dependencies, deferred Tempo stance.
- [`subgraphs/pearl/pearl-trades-schema.md`](subgraphs/pearl/pearl-trades-schema.md) — proposed schema delta for the generalized `predict-polymarket`: adds `ServiceClassification` + `ClassificationChange` entities, `classification` link + optional `ownerEOA`/`agentEOA` on `TraderAgent`, cohort-scoped `Funding`/`FundingDaily`, `PayoutSource` enum on `PayoutRedemption`. Parallel-deployment cutover, no schema mutation of the running deployment.
