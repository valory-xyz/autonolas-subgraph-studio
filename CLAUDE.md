...
- [`subgraphs/pearl/SUBGRAPH_PLAN.md`](subgraphs/pearl/SUBGRAPH_PLAN.md) — Pearl Mini analytics via in-place generalization of `predict-polymarket` (Polygon). Cohort filtering is client-side against `agentIds` and `AgentInstance.operator`; `service-registry` reused for service enumeration; no dedicated Pearl subgraph. `ApplicationClassifier` (from `valory-xyz/autonolas-registries`) is a documented future enhancement, not current scope. Funding flows and owner-EOA tracking explicitly out of scope.
- [`subgraphs/pearl/pearl-trades-schema.md`](subgraphs/pearl/pearl-trades-schema.md) — schema delta for the generalized `predict-polymarket`: adds `AgentInstance` entity (captures `RegisterInstance` tuples), `agentIds: [Int!]!` on `TraderAgent`, `PayoutSource` enum on `PayoutRedemption` for NegRisk/vanilla distinction. Parallel-deployment cutover, no mutation of the running deployment.
