...
import { getBalancerVaultAddress, getOlasStablePoolAddress, getOlasTokenAddress, getStableTokenAddress, getBurnAddressMechFees, CHAINLINK_PRICE_FEED_ADDRESS_ARBITRUM_ETH_USD } from "../../../../shared/constants";
...
const STABLE_ADDRESS = getStableTokenAddress(); // WETH on Arbitrum
const PRICE_FEED_ADDRESS = Address.fromString(CHAINLINK_PRICE_FEED_ADDRESS_ARBITRUM_ETH_USD);
...
export function handleMechBalanceAdjustedForTokenOlas(event: MechBalanceAdjusted): void {
  ...
  const ethValueInWei = deliveryRateInEth
    ...
    .truncate(0).digits;
  const deliveryRateUsd = convertArbitrumNativeWeiToUsd(
...
export function handleWithdrawForTokenOlas(event: Withdraw): void {
  ...
  const ethValueInWei = withdrawalAmountInEth
    .times(BigInt.fromI32(10).pow(ETH_DECIMALS as u8).toBigDecimal())
    .truncate(0).digits;
  const withdrawalAmountUsd = convertArbitrumNativeWeiToUsd(
    BigInt.fromString(ethValueInWei.toString()),
    latestRoundData.value.value1
  );
