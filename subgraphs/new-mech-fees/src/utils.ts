...
// For native fees on Gnosis (xDAI = 1 USD)
...
// For native fees with Chainlink price feed (Base/Polygon/Optimism)
export function convertNativeWeiToUsd(
  amountInWei: BigInt,
  nativePrice: BigInt
): BigDecimal {
  ...
  return amountInWei
    .toBigDecimal()
    .times(nativePrice.toBigDecimal())
    .div(priceDivisor)
    .div(ethDivisor);
}
...
// For USDC amounts (assumes 1 USDC = 1 USD)
