 ...
 *
 * Example:
 *   node scripts/validate-global.js https://api.studio.thegraph.com/query/xxx/predict-omen/version/latest
 */
...
async function paginate(url, entityName, fields, orderBy = "blockTimestamp", pageSize = 1000) {
  let all = [];
  let lastId = "";
  while (true) {
...
async function main() {
  ...
  const bigIntFields = [
    "totalTraded", "totalFees", "totalTradedSettled",
    "totalFeesSettled", "totalPayout", "totalExpectedPayout",
  ];
  ...
  const gFees = bn(global.totalFees);
  const gFeesSettled = bn(global.totalFeesSettled);
  ...
  const invariants = [
    ...
    { name: "totalFeesSettled <= totalFees", ok: gFeesSettled <= gFees, detail: `${fmt(gFeesSettled)} vs ${fmt(gFees)}` },
  ...
  if (unsettled > 0n) {
    console.log(`\n  Info: ${fmt(unsettled)} xDAI in unsettled trades (open markets)`);
  }
